...
import {quantizeScale, getQuantileScale} from '@deck.gl/aggregation-layers/utils/scale-utils';
...
test.only('scale-utils#quantileScale', t => {
  for (const tc of QUANTILE_SCALE_TEST_CASES) {
    ...
    for (const i in tc.values) {
      ...
      t.deepEqual(result, tc.results ? tc.results[i] : tc.result, `quantileScale ${tc.title} returned expected value`);
    }
  }
  t.end();
});
