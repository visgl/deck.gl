...
test('DataManager#subscribe, unsubscribe', t => {
  ...
  t.throws(
    () => dataManager.subscribe('resource://data-01'),
    /data-01 not found/,
  ...
  t.ok(propA1Changed === 0 && propA2Changed === 0 && propBChanged === 0, 'data did not change');

  dataManager.add('data-01', data1, {persistent: false, forceUpdate: true});
  t.ok(propA1Changed === 1 && propA2Changed === 1 && propBChanged === 0, 'data-01 changed');

  data2 = ['a', 'b'];
  dataManager.add('data-02', data2, {persistent: true});
  t.ok(propA1Changed === 1 && propA2Changed === 1 && propBChanged === 1, 'data-02 changed');

  t.is(dataManager.subscribe('resource://data-01', () => propBChanged++, 'Layer2', 'propB'), data1);
  data2 = ['a'];
  dataManager.add('data-02', data2, {persistent: true});
  t.ok(
    propA1Changed === 1 && propA2Changed === 1 && propBChanged === 1,
    'data-02 no longer listened to'
  );

  dataManager.add('data-01', data1, {persistent: false, forceUpdate: true});
  t.ok(propA1Changed === 2 && propA2Changed === 2 && propBChanged === 2, 'data-01 changed');

  t.is(dataManager.subscribe('http://data.json', () => propBChanged++, 'Layer2', 'propB'), null);
  dataManager.add('data-01', data1, {persistent: false, forceUpdate: true});
  t.ok(propA1Changed === 3 && propA2Changed === 3 && propBChanged === 2, 'data-01 changed');
