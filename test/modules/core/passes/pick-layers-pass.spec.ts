...
test('PickLayersPass#render sequence shares layer colors and preserves earlier pixels', () => {
  const pickingFBO = device.createFramebuffer({colorAttachments: ['rgba8unorm']});
  pickingFBO.resize({width: 100, height: 100});

  const firstView = new MapView({id: 'first'});
  const secondView = new MapView({id: 'second'});
  const viewState = {longitude: 0, latitude: 0, zoom: 1};
  const firstViewport = firstView.makeViewport({width: 100, height: 100, viewState});
  const secondViewport = secondView.makeViewport({width: 100, height: 100, viewState});
  const firstLayer = new ScatterplotLayer({
    id: 'first-layer',
    data: [{position: [-10, 0]}],
    getPosition: datum => datum.position,
    radiusMinPixels: 5,
  ...
  const secondLayer = new ScatterplotLayer({
    id: 'second-layer',
    data: [{position: [10, 0]}],
    getPosition: datum => datum.position,
    radiusMinPixels: 5,
  ...
  const layerManager = new LayerManager(device, {viewport: firstViewport});
  const pickLayersPass = new PickLayersPass(device);

  try {
    layerManager.setLayers([firstLayer, secondLayer]);

    const renderOptions = {
      pass: 'picking',
      pickZ: false,
      viewports: [firstViewport],
      views: {first: firstView, second: secondView},
      layers: layerManager.getLayers(),
      layerFilter: ({layer, viewport}) => layer.id === `${viewport.id}-layer`,
      onViewportActive: layerManager.activateViewport,
      pickingFBO,
      deviceRect: {x: 0, y: 0, width: 100, height: 100}
    };

    pickLayersPass.beginRenderSequence(renderOptions);
    let decodePickingColor;
    try {
      pickLayersPass.render(renderOptions);
      ({decodePickingColor} = pickLayersPass.render({
        ...renderOptions,
        viewports: [secondViewport],
        clearCanvas: false,
        clearStack: false
      }));
    } finally {
      pickLayersPass.endRenderSequence();
    }
    ...
    const firstPixelIndex = (50 * 100 + Math.round(firstViewport.project([-10, 0])[0])) * 4;
    const secondPixelIndex = (50 * 100 + Math.round(secondViewport.project([10, 0])[0])) * 4;
    const firstPickedObject = decodePickingColor!(
      pixels.slice(firstPixelIndex, firstPixelIndex + 4)
    );
    const secondPickedObject = decodePickingColor!(
      pixels.slice(secondPixelIndex, secondPixelIndex + 4)
    );

    expect(firstPickedObject?.pickedLayer.id, 'the first pass color survives the second pass').toBe(
      firstLayer.id
    );
    expect(
      secondPickedObject?.pickedLayer.id,
      'the second pass receives its own picking color'
    ).toBe(secondLayer.id);
    expect(firstPickedObject?.pickedViewports).toEqual([firstViewport]);
    expect(secondPickedObject?.pickedViewports).toEqual([secondViewport]);
  } finally {
    layerManager.finalize();
    pickingFBO.destroy();
  }
