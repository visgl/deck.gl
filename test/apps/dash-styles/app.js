...
function straightPath(segments, latitude) {
  const path = [];
  for (let i = 0; i <= segments; i++) {
    path.push([-122.48 + (SPAN_LNG * i) / segments, latitude]);
  }
  return path;
}

/** A line that climbs in Z, so 2D and 3D arclength disagree. */
function climbingPath(segments, latitude, height) {
  const path = [];
  for (let i = 0; i <= segments; i++) {
    const t = i / segments;
    path.push([-122.48 + SPAN_LNG * t, latitude, height * t]);
  }
  return path;
}

/** A circle of short chords - the dense-polyline case that renders solid today. */
function circlePath(segments, center, radius) {
  const path = [];
  for (let i = 0; i <= segments; i++) {
...
const ROWS = [
  ['1 segment', lat => straightPath(1, lat)],
  ['4 segments', lat => straightPath(4, lat)],
  ['20 segments', lat => straightPath(20, lat)],
  ['120 segments', lat => straightPath(120, lat)],
  ['3D climb', lat => climbingPath(40, lat, 1500)]
];
...
const CONTROLS = [
  ...
  {key: 'width', type: 'range', label: 'stroke width', min: 1, max: 40, step: 1},
  {
    ...
    type: 'select',
    label: 'dash mode',
    // 'path' only exists once layer 4 lands; before that it falls back to 'segment'.
...
function dashExtension() {
  // `dashMode` is only understood once layer 4 lands. Probe the constructor so this app
  // keeps working against every layer of the stack rather than throwing on older builds.
  try {
    ...
    billboardVariants().forEach((billboard, variantIndex) => {
      layers.push(
        new PathLayer({
          ...
          data: [
            circlePath(120, [center[0] + (variantIndex - 0.5) * 0.03, center[1]], ROW_SPACING * 1.3)
          ],
          getPath: d => d,
          billboard,
          widthUnits: state.widthUnits,
