...
const getTooltip: InfoWidgetProps['getTooltip'] = (info, widget) => {
...
function App() {
  const [layers, setLayers] = useState(() => getLayers());
  const pinRef = useRef<HTMLElement | null>(null);

  if (!pinRef.current) {
    pinRef.current = createPin();
  }
  ...
  return (
    <DeckGL
      ...
      layers={layers}
    >
      <_GeocoderWidget geocoder="coordinates" _geolocation />
      <ZoomWidget />
      <CompassWidget />
      <FullscreenWidget />
      <ScreenshotWidget />
      <ResetViewWidget />
      <LoadingWidget />
      <_ScaleWidget placement="bottom-right" />
      <ThemeWidget />
      <ContextMenuWidget
      ...
      />
      <InfoWidget mode="hover" getTooltip={getTooltip} arrow={10} offset={10} />
      <PopupWidget
        position={[-5, 52]}
        marker={{element: pinRef.current}}
        placement="top"
        offset={20}
        content="I'm here!"
        closeOnClickOutside
      ...
      <_TimelineWidget
        ...
        step={1}
        playInterval={500}
        onTimeChange={time => setLayers(getLayers([2, time]))}
      />
      <_StatsWidget type="deck" />
      <IconWidget
        placement="top-right"
        label="Run!"
      ...
      <ToggleWidget
        ...
        onIcon='data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" height="100%" viewBox="0 -960 960 960" width="100%" fill="currentColor"><path d="m233-120 65-281L80-590l288-25 112-265 112 265 288 25-218 189 65 281-247-149-247 149Z"/></svg>'
        label="Favorite"
        onLabel="Cancel"
        onColor="skyblue"
