...
Pan and zoom anchors are computed on the common-space plane Z `0`. The controller never needs `projection.inverse`, so navigation remains usable outside the inverse domain. The center's Z component is always reset to zero.

Bounds constrain the center and impose a minimum zoom to fit the unrotated viewport within the bounds. This fit uses neither pitch nor bearing, so rotation does not move the map merely to satisfy bounds. An explicit `maxZoom` caps the fit zoom. Bounds can prevent a dragged or zoomed anchor from staying exactly under the pointer; disable bounds when unrestricted anchoring is required.
