...
* `inertia` (boolean | number) - Enable inertia after pointer panning/rotation and trackpad pinch. Touch pinch ends at the last live zoom to avoid lift-frame jumps and allow immediate gesture handoff. If a number is provided, indicates the duration of time over which the velocity reduces to zero, in milliseconds. Default `false`.
