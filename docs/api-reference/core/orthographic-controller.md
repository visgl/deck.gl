...
- `maxBoundsRubberBand` (boolean) - allows panning to temporarily overshoot `maxBounds` with increasing resistance, then animates the viewport back within bounds on release. Requires `maxBounds` and does not relax zoom constraints. Uses the configured `inertia` duration, or a 300 ms default, for the return transition. Default `false`.
