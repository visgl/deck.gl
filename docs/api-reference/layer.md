...
##### `data` (Array or String, optional)

* Default: `[]`

deck.gl layers typically expect the value of the `data` prop to be a JavaScript array.

Alternatively, it is possible to supply a `String` containing a URL as the value of the `data` prop. In this case deck.gl will attempt to load the URL, parse it as JSON and then use the expected resulting JavaScript Array as its `data` prop.
