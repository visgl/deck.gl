...
This release improves WebGPU attribute-buffer handling, adds WebGPU render-test coverage, and makes switching between WebGL2 and WebGPU more reliable in React and across website examples. [`MVTLayer`](./api-reference/geo-layers/mvt-layer.md) now renders on WebGPU using luma.gl shader plugins to apply tile clipping to its circle, path, and polygon sublayers.
