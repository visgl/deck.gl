...
- [`PathStyleExtension`](./api-reference/extensions/path-style-extension.md) dashes render the same in `GlobeView` as in `MapView` for every `dashMode` and `dashUnits`, verified by side-by-side render tests.
