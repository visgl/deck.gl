...
`Layer`'s name (defined by the `layerName`. If you have more than one instance of the same
...
will be generated through appending the sub layer's id to the parent layer
(i.e. the composite layer) `id`, which helps avoid `id` collisions in this case.
...
* For React users: `id` is similar to the `key` property used in React to match
  ...
  the `id` whereas in React the `key` is optional but strongly recommended.
