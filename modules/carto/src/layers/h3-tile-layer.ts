  ...
  renderLayers(): Layer | null | LayersList {
    ...
    if (this.props.minZoom) {
      minresolution = Math.max(
        minresolution,
        getHexagonResolution({zoom: this.props.minZoom, latitude: 0, tileSize: this.props.tileSize})
      );
    }
    if (this.props.maxZoom) {
      maxresolution = Math.min(
        maxresolution,
        getHexagonResolution({zoom: this.props.maxZoom, latitude: 0, tileSize: this.props.tileSize})
      );
    }
