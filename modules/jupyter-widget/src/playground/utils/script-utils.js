...
// do not emit "load" events. `detail` is `{key, ok, error}`; `ok` is false when the import or the
// module's top-level evaluation threw, with `error` carrying the (possibly empty) message.
...
export function loadModule(url, globalName) {
  ...
  if (!scriptLoadPromises[key]) {
    ...
    const dispatch = (ok, error) =>
      `document.dispatchEvent(new CustomEvent(${quote(MODULE_LOADED_EVENT)}, ` +
      `{detail: {key: ${quote(key)}, ok: ${ok}, error: ${error}}}))`;
    const script = document.createElement('script');
    script.type = 'module';
    // A dynamic import inside try/catch reports fetch, parse and top-level evaluation failures alike
    script.textContent =
      `try { const m = await import(${quote(url)}); window[${quote(globalName)}] = m; ${dispatch(true, 'null')}; } ` +
      `catch (error) { ${dispatch(false, 'String((error && error.message) || error)')}; }`;
