...
The affected layers write a flat color per fragment. Path and line strokes end in
`fragColor = vColor`, ArcLayer emits a triangle strip with hard side edges, and PointCloudLayer
hard-discards outside each point's circular silhouette. There is no coverage computation in any of
them, so edge quality is inherited from the render target.
...
- **`@deck.gl/google-maps`, interleaved.** deck attaches to the context handed to
  ...
  and not determinable from deck's source, but the behavior is established by report:
...
`PointCloudLayer` expose the primitive prop directly. The prop name, default-off ergonomics and
documentation register follow the existing `ScatterplotLayer.antialiasing` precedent.
...
plumbing problem from the coverage calculation: the `project` shader module is registered for the
vertex stage only, so `project.devicePixelRatio` is not reachable from the fragment shader. The
derivative approach needs no DPR varying. The vertex stage uses the project uniform only to convert
the half-device-pixel envelope padding to CSS pixels.
...
- **`PathStyleExtension` offset.** The extension hard-`discard`s outside `|vPathPosition.x| > 1`
  ...
  boundary and then cuts. Measured at 0.447 of the un-offset feather, versus 0.328 when the pixel
  scale was incorrectly derived from stroke width. This improves
...
**Golden images** — the focused `test/render/test-cases/*-antialiasing.spec.ts` suites. A golden diff can cover this,
...
With all three, disabling the feather drops each primitive layer below its 99.8% threshold.
...
- Deleting the feather drops the antialiased pass from 1572 partial pixels to 0.
- Removing the half-device-pixel geometry envelope clips the outer coverage ramp: the minimum
  nonzero alpha jumps to 128. The PathLayer, LineLayer, and ArcLayer assertions require alpha below
  96, which verifies that fragments outside the declared edge are rasterized.
- Restoring the previous varying-based implementation reproduces the 0.328 offset collapse.

Equivalent no-MSAA framebuffer assertions for LineLayer, ArcLayer, and PointCloudLayer verify that
those silhouettes gain continuous partial coverage without relying on the path implementation.

**Premultiplied alpha** — the focused `*-webgpu-antialiasing.spec.ts` tests. WebGPU blends
premultiplied, so coverage has to reach alpha *before* `deckgl_premultiplied_alpha`; applying it
after would leave RGB too bright for its alpha, a light halo on every edge. The tests render each
primitive layer on WebGPU into an offscreen framebuffer and assert red tracks alpha at
partial-coverage pixels. Moving the multiply after premultiplication takes the worst overshoot from
2 to 126.

**WebGPU golden coverage** uses offscreen framebuffer assertions rather than screenshots. The
dedicated antialiasing goldens run on WebGL because the WebGPU canvas does not present under the
headless software renderer CI uses, so its screenshot comes back blank whatever deck draws.
Reading back an offscreen framebuffer sidesteps canvas presentation and verifies PathLayer,
LineLayer, ArcLayer, and PointCloudLayer on WebGPU. Enable WebGPU golden rows once CI has hardware
WebGPU.
