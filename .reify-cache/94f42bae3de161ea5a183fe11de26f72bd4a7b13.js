"use strict";module.export({default:()=>GPUGridAggregator});var GL;module.link('@luma.gl/constants',{default(v){GL=v}},0);var Buffer,Model,Transform,FEATURES,hasFeatures,isWebGL2,readPixelsToBuffer,fp64Utils,withParameters;module.link('@luma.gl/core',{Buffer(v){Buffer=v},Model(v){Model=v},Transform(v){Transform=v},FEATURES(v){FEATURES=v},hasFeatures(v){hasFeatures=v},isWebGL2(v){isWebGL2=v},readPixelsToBuffer(v){readPixelsToBuffer=v},fp64(v){fp64Utils=v},withParameters(v){withParameters=v}},1);var log;module.link('@deck.gl/core',{log(v){log=v}},2);var worldToPixels;module.link('viewport-mercator-project',{worldToPixels(v){worldToPixels=v}},3);var AGGREGATION_OPERATION,DEFAULT_CHANGE_FLAGS,DEFAULT_RUN_PARAMS,MAX_32_BIT_FLOAT,MIN_BLEND_EQUATION,MAX_BLEND_EQUATION,MAX_MIN_BLEND_EQUATION,EQUATION_MAP,ELEMENTCOUNT,DEFAULT_WEIGHT_PARAMS,IDENTITY_MATRIX,PIXEL_SIZE,WEIGHT_SIZE;module.link('./gpu-grid-aggregator-constants',{AGGREGATION_OPERATION(v){AGGREGATION_OPERATION=v},DEFAULT_CHANGE_FLAGS(v){DEFAULT_CHANGE_FLAGS=v},DEFAULT_RUN_PARAMS(v){DEFAULT_RUN_PARAMS=v},MAX_32_BIT_FLOAT(v){MAX_32_BIT_FLOAT=v},MIN_BLEND_EQUATION(v){MIN_BLEND_EQUATION=v},MAX_BLEND_EQUATION(v){MAX_BLEND_EQUATION=v},MAX_MIN_BLEND_EQUATION(v){MAX_MIN_BLEND_EQUATION=v},EQUATION_MAP(v){EQUATION_MAP=v},ELEMENTCOUNT(v){ELEMENTCOUNT=v},DEFAULT_WEIGHT_PARAMS(v){DEFAULT_WEIGHT_PARAMS=v},IDENTITY_MATRIX(v){IDENTITY_MATRIX=v},PIXEL_SIZE(v){PIXEL_SIZE=v},WEIGHT_SIZE(v){WEIGHT_SIZE=v}},4);var AGGREGATE_TO_GRID_VS;module.link('./aggregate-to-grid-vs.glsl',{default(v){AGGREGATE_TO_GRID_VS=v}},5);var AGGREGATE_TO_GRID_VS_FP64;module.link('./aggregate-to-grid-vs-64.glsl',{default(v){AGGREGATE_TO_GRID_VS_FP64=v}},6);var AGGREGATE_TO_GRID_FS;module.link('./aggregate-to-grid-fs.glsl',{default(v){AGGREGATE_TO_GRID_FS=v}},7);var AGGREGATE_ALL_VS_FP64;module.link('./aggregate-all-vs-64.glsl',{default(v){AGGREGATE_ALL_VS_FP64=v}},8);var AGGREGATE_ALL_FS;module.link('./aggregate-all-fs.glsl',{default(v){AGGREGATE_ALL_FS=v}},9);var TRANSFORM_MEAN_VS;module.link('./transform-mean-vs.glsl',{default(v){TRANSFORM_MEAN_VS=v}},10);var getFloatTexture,getFramebuffer,getFloatArray,updateBuffer;module.link('./gpu-grid-aggregator-utils.js',{getFloatTexture(v){getFloatTexture=v},getFramebuffer(v){getFramebuffer=v},getFloatArray(v){getFloatArray=v},updateBuffer(v){updateBuffer=v}},11);
