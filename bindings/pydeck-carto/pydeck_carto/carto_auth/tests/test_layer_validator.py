...
@pytest.mark.parametrize('layer_type, must_error', (
        (MapType.QUERY, False),
        (MapType.TABLE, False),
...
def test_check_credential_token():
    ...
    with pytest.raises(ValueError):
        ...
        validity = is_valid_carto_layer(layer, ca)


@pytest.mark.parametrize('layer_geo_column, must_error', (
        (GeoColumnType.H3, False),
        (GeoColumnType.QUADBIN, False),
        ('RandomName', True),
        (pdk.types.String("randomColumn"), True),
))
...
def test_layers_missing_attrs(carto_dummy_auth):
    for mandatory_attr in ['data', 'type_', 'connection', 'credentials']:
        params = dict(data="SELECT geom, name FROM carto-demo-data.demo_tables.airports",
                      type_=MapType.QUERY,
                      connection=CartoConnection.CARTO_DW,
                      credentials=carto_dummy_auth.get_layer_credentials(), )
...
def test_list_connections(requests_mock, carto_dummy_auth):
    requests_mock.get(
        ...
        json=[
            {'name': 'my_own_connection', 'provider_id': 'bigquery',
             'config': {'type': 'oauth_token',
                        'credentials': {
                            'picture': 'picture-url',
                            'expires_in': 1661331110615,
                            'client_email': 'super-email@exqample.com'},
                        'billing_project': 'my-billing'},
             'user_id': 'google-oauth2|12345', 'account_id': 'ac_custom',
             'created_at': '2022-08-24T07:52:22.169Z', 'updated_at': '2022-08-24T10:50:56.468Z',
             'id': '8b9d630e-ddb5-4796-8a16-d2323072fbbb', 'spatial_extension': False,
             'privacy': 'private',
    ...
    conns = carto_dummy_auth.connections_list()
    assert isinstance(conns, list)

    random_str_connection = 'myrandom_connnection'
    assert carto_dummy_auth.is_valid_connection(random_str_connection) is False

    first_connection = 'my_own_connection'
    assert carto_dummy_auth.is_valid_connection(first_connection) is True
