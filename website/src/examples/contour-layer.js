...
import App, {BANDS} from 'website-examples/contour/app';
    ...
    return (
      <div>
        ...
        <div className="layout">
          {BANDS.map((c, i) => <div className="legend" key={i}
            style={{
        ...
        <p className="layout">
        {BANDS.map((c, i) => <div key={i} className="text-right"
            style={{width: `${100 / BANDS.length}%`}} >{i < BANDS.length - 1 ? c.threshold[1] : ''}</div>)}
        </p>
        <p>
  ...
  updateOptions() {
    ...
    if (data) {
      ...
      this.props.useParam({
        ...
        week: {displayName: 'Week', type: 'range', value: maxWeek, step: 1, min: 0, max: maxWeek}
      });
    }
  }
