...
void main(void) {
  ...
  vec2 ptPosition = instancePositions.xy + vec2((positions.x + 1.0 ) * lonOffset / 2., (positions.y + 1.) * latOffset / 2.);

  vec2 pos = project_position(ptPosition);

  // if 3d not enabled set elevation to 0
  float elevation =  mix(0., project_scale(instancePositions.w  * (positions.z + 1.) * ELEVATION_SCALE), extruded);
  ...
  vec3 lightWeightedColor = mix(vec3(1.), lightWeighting, extruded) * (instanceColors.rgb / 255.);
  ...
  gl_Position = project_to_clipspace(vec4(extrudedPosition, 1.));
}
